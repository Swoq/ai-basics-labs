...
class App:
            ...
            for yidx, line in enumerate(file):
                for xidx, char in enumerate(line):
                    if char == "#":
                        ...
                        self.player_position = [xidx, yidx]
                    elif char in ['0', '1', '2', '3']:
                        if self.enemies_position.get(char) is None:
                            self.enemies_position[char] = [[xidx, yidx]]
                        else:
                            self.enemies_position.get(char).append([xidx, yidx])
    ...
    def make_enemies(self):
        self.enemies_position.pop("")
        for key, val in self.enemies_position.items():
            for pair in val:
                self.enemies.append(Enemy(self, vec(pair), int(key)))
